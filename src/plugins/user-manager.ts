...
export function userManagerPlugin(): Plugin {
  return {
    ...
    activate(ctx) {
      ...
      db.run(`CREATE TABLE IF NOT EXISTS current_user (
      ...
      if (userCount === 0 && existsSync(legacyCatalog)) {
        ...
        db.run(
          ...
          [defaultUser.id, new Date().toISOString()],
        );
      }
      ...
      ctx.provide('user.manager', {
        ...
        getCurrentId(): string | null {
          const row = db.query('SELECT user_id FROM current_user WHERE singleton = 1').get() as { user_id: string } | null;
...
function createUserInternal(db: Database, usersDir: string, name: string): UserRecord {
  const sanitized = name.toLowerCase().replace(/[^a-z0-9_-]/g, '-').replace(/-+/g, '-');
  if (!sanitized) throw new Error('Invalid user name');

  const existing = db.query('SELECT id FROM users WHERE name = ?').get(sanitized) as { id: string } | null;
  ...
  db.run(
    ...
    [id, sanitized, createdAt, dataDir],
  );
